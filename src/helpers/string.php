...
function format_minutes_hours($minutes, $minified = false) {
	$seconds = $minutes * 60;
...
function implode_natural(array $list, $conjunction = 'and') {
	$last = array_pop($list);
	if ($list) {
...
if (!function_exists('upper_camel_case')) {
	function upper_camel_case($value) {
		return ucfirst(camel_case($value));
	}
}

if (!function_exists('str_number_only')) {
	/**
	 ...
	 * @return mixed|null
	 */
	function str_number_only($str, $thousandSeparator = ',', $decimalMark = '.') {
